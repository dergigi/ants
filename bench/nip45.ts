  ...
  while (Date.now() < deadline) {
    ...
    if (ndkInstance.pool?.relays) {
      Array.from(ndkInstance.pool.relays.values()).forEach(r => {
        if (r.status === NDK_RELAY_STATUS_CONNECTED) count++;
      });
    }
    if (count > 0) {
      ...
      if (ndkInstance.pool?.relays) {
        Array.from(ndkInstance.pool.relays.values()).forEach(r => {
          if (r.status === NDK_RELAY_STATUS_CONNECTED) final++;
        });
      }
      return final;
    }
...
function runSearch(relayUrls: string[], query: string): Promise<SearchResult> {
  return new Promise(async (resolve) => {
    const start = performance.now();
