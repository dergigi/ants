...
import { SPELL_KIND } from './spellTranslate';

/** Parsed spell display data extracted directly from event tags */
...
/**
 * Parse a kind:777 event into display data.
 * Reads raw tags without resolution for safe UI rendering.
 */
export function parseSpellDisplay(event: NDKEvent): SpellDisplayData | null {
  ...
  const variables = allValues.filter((v) => typeof v === 'string' && v.startsWith('$'));
  const name = nameTag?.[1] || (event.content ? event.content.slice(0, 80) : 'Unnamed spell');

  const parsedLimit = limitTag ? parseInt(limitTag[1], 10) : undefined;
