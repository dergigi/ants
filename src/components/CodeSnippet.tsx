    ...
    <div className={className || ''}>
      ...
      <Highlight code={code} language={(normalizedLanguage || 'tsx') as Language} theme={themes.nightOwl}>
        {({ className: cls, style, tokens, getLineProps, getTokenProps }: RenderProps) => (
          <pre
            className={`${cls} text-sm overflow-x-auto rounded-md p-3 bg-[#0f0f0f] border border-[#3d3d3d]`.trim()}
        ...
        <pre className="text-sm overflow-x-auto rounded-md p-3 bg-[#0f0f0f] border border-[#3d3d3d] text-gray-100 whitespace-pre-wrap break-words">
          {code}
        </pre>
      )}
      {(() => {
