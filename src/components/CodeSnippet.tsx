      ...
      <div className="text-xs text-gray-300 mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          <span className="font-semibold truncate">
            {snippetName ? `${snippetName} (${language ? `code:${language}` : 'code'})` : (language ? `code:${language}` : 'code')}
          </span>
        </div>
        {headerRight}
      </div>
      ...
      <Highlight code={code} language={(normalizedLanguage || 'tsx') as Language} theme={themes.nightOwl}>
        {({ className: cls, style, tokens, getLineProps, getTokenProps }: RenderProps) => (
          <pre
            ...
            style={{ ...style, background: 'rgba(31, 31, 31, 0.5)', whiteSpace: 'pre' }}
          >
            {tokens.map((line, i) => (
        ...
        <pre className="text-sm overflow-x-auto rounded-md p-3 border border-[#3d3d3d] text-gray-100 whitespace-pre-wrap break-words" style={{ background: 'rgba(31, 31, 31, 0.5)' }}>
          {code}
        </pre>
      )}
      {(() => {
